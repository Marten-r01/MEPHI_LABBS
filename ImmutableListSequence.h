...
class ImmutableListSequence :ListSequence<T> {
	...
	virtual ListSequence<T> Append(const T& item) const override {
		ImmutableListSequence<T>* new_list = new ImmutableListSequence<T>(*this);
		new_list->Append(item);
		return new_list;
	}
	virtual ListSequence<T> Prepend(const T& item) const override {
		ImmutableListSequence<T>* new_list = new ImmutableListSequence<T>(*this);
		new_list->Prepend(item);
		return new_list;
	}
	virtual ListSequence<T> RemoveAt(int index) const override {
		ImmutableListSequence<T>* new_list = new ImmutableListSequence<T>(*this);
		new_list->RemoveAt(index);
		return new_list;
	}
	virtual ListSequence<T> InsertAt(const T& item, int index) const override {
		ImmutableListSequence<T>* new_list = new ImmutableListSequence<T>(*this);
